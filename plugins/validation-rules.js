export default ({ store }, inject) => {
  inject('rules', {
    ...
    greaterThan: (threshold) => { return value => value > threshold || `Wartość musi być większa niż ${threshold}` },
    greaterThanEquals: (threshold) => { return value => value >= threshold || `Wartość musi być nie mniejsza niż ${threshold}` },
    lowerThanEquals: (threshold) => { return value => value <= threshold || `Wartość musi być nie większa niż ${threshold}` }
  })
}
